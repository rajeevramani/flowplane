...
- [x] S12 Hardening, production readiness, v1.0.0 tag
  ...
  - [x] S12 preflight CI gates: #88 restores required quality and supply-chain jobs. The quality
        ...
        reflects the resolved Q-006 posture: first-party crates are Apache-2.0 and unpublished
        (`publish = false`), configured as private in cargo-deny.
  ...
  - [x] S12e packaging (greenfield release infrastructure): #93 adds `scripts/package-release.sh`,
        `Containerfile.release`, `.dockerignore`/`.containerignore`, and `docs/release-packaging.md`.
        The v1.0 artifact decision is musl-capable static release builds via vendored OpenSSL, with
        certificate-stack replacement deferred post-1.0. The package command produces host or
        `FLOWPLANE_RELEASE_TARGET` binaries, cargo-metadata SBOM source, tarball, manifest, and
        checksums; `FLOWPLANE_PACKAGE_IMAGE=1` builds/saves the OCI image when Docker/Podman has
        free builder storage. Dataplane bootstrap remains generated by the shipped
        `flowplane dataplane bootstrap --mode mtls` command against a live CP. Q-006 is resolved
        (Apache-2.0); public distribution is no longer license-gated.
  - [x] S12f operator docs: #94 adds `docs/production-readiness.md` as the production entry
