...
impl ClusterRepository {
    ...
    pub async fn get_by_id(&self, id: &str) -> Result<ClusterData> {
        ...
        match row {
            ...
            None => Err(FlowplaneError::not_found_msg(format!("Cluster with ID '{}' not found", id))),
        }
    }
    ...
    pub async fn get_by_name(&self, name: &str) -> Result<ClusterData> {
        ...
        match row {
            ...
            None => Err(FlowplaneError::not_found_msg(format!("Cluster with name '{}' not found", name))),
        }
    }
    ...
    pub async fn list(&self, limit: Option<i32>, offset: Option<i32>) -> Result<Vec<ClusterData>> {
        ...
        let rows = sqlx::query!(
            r#"
            SELECT id, name, service_name, configuration, version, created_at, updated_at
            FROM clusters
    ...
    pub async fn update(&self, id: &str, request: UpdateClusterRequest) -> Result<ClusterData> {
        ...
        let result = sqlx::query!(
            r#"
            UPDATE clusters
            SET service_name = ?1, configuration = ?2, version = ?3, updated_at = ?4
            WHERE id = ?5
            "#,
            new_service_name,
            new_configuration,
        ...
        if result.rows_affected() == 0 {
            return Err(FlowplaneError::not_found_msg(format!("Cluster with ID '{}' not found", id)));
        }
    ...
    pub async fn exists_by_name(&self, name: &str) -> Result<bool> {
        let count = sqlx::query_scalar!(
            "SELECT COUNT(*) as count FROM clusters WHERE name = ?1",
            name
        )
        .fetch_one(&self.pool)
        .await
    ...
    /// Access the underlying database pool (used by background tasks/watchers)
...
mod tests {
    ...
    async fn test_cluster_validation() {
        ...
        // Create cluster with invalid JSON should work (we serialize it)
        let create_request = CreateClusterRequest {
            name: "test_cluster".to_string(),
            service_name: "test_service".to_string(),
            configuration: serde_json::json!({"valid": "json"}),
        };
