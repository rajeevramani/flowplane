...
/// **DEPRECATED**: Use `POST /api/v1/api-definitions/from-openapi` instead.
///
/// This endpoint creates native resources directly without FK tracking or source tagging.
/// The new Platform API endpoint provides better resource management, automatic bootstrap
/// generation, and unified data model.
pub async fn create_gateway_from_openapi_handler(
    ...
    warn!(
        "Deprecated endpoint called: POST /api/v1/gateways/openapi. \
        Use POST /api/v1/api-definitions/from-openapi instead for better resource management."
    );
