...
/// Verify the user has resource-level access to the specified team for custom-wasm-filters
fn require_team_resource_access(
    context: &AuthContext,
    team: &str,
    action: &str,
) -> Result<(), ApiError> {
...
pub async fn create_custom_wasm_filter_handler(
    ...
    require_team_resource_access(&context, &team, "write")?;
