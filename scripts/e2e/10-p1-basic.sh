...
./target/debug/flowplane -o table stats overview >/tmp/fp-e2e-stats.txt   # -o table: non-TTY stdout defaults to JSON (CLI-R-11); fpv2-xfh
grep -q "TOTAL DATAPLANES" /tmp/fp-e2e-stats.txt || fail "stats overview did not render dataplane totals"
./target/debug/flowplane -o table ops xds status >/tmp/fp-e2e-xds-status.txt
grep -q "TOTAL DATAPLANES" /tmp/fp-e2e-xds-status.txt || fail "xds status did not render dataplane totals"
./target/debug/flowplane -o table ops xds nacks >/tmp/fp-e2e-xds-nacks.txt
