...
| `FLOWPLANE_CERT_ISSUER_CA_CERT_PATH` | server | — | for issuance ²¹ | Issuing CA certificate PEM path. A standards-complete intermediate may be the explicit trust anchor. |
...
| `FLOWPLANE_RLS_ADMIN_URL` | server | — | no ²⁰ | HTTP(S) admin URL of the rate-limit service. When set, the CP starts the `rls_sync` worker that pushes the full policy set to the RLS on the reconcile loop. |
| `FLOWPLANE_RLS_ADMIN_TOKEN` | server | — | no ²⁰ | Bearer credential the CP presents on every `rls_sync` push (`Authorization: Bearer …`). Mutually exclusive with `_FILE`. **Fail-closed**: with a token set, a non-loopback `http://` admin URL refuses startup. |
| `FLOWPLANE_RLS_ADMIN_TOKEN_FILE` | server | — | no ²⁰ | Path to a file holding the CP-side push credential (trailing newline trimmed). Mutually exclusive with `FLOWPLANE_RLS_ADMIN_TOKEN`. |
| `FLOWPLANE_RLS_ADMIN_TLS_CA` | server | — | no ²⁰ | CA bundle the CP verifies the RLS **admin server certificate** against (private-CA deployments). Unset ⇒ system roots. Unreadable/non-PEM/empty bundle fails startup. |
| `FLOWPLANE_RLS_ALLOW_INSECURE_ADMIN_PUSH` | server | — | no ²⁰ | Explicit acknowledgement (`yes-this-is-local-only`) that the CP may send the push bearer over **plaintext to a loopback-literal** admin URL (dev only). Never unlocks a non-loopback URL. |
...
| ²⁰ | `FLOWPLANE_RLS_ADMIN_TOKEN[_FILE]`, `FLOWPLANE_RLS_ADMIN_TLS_CA`, `FLOWPLANE_RLS_ALLOW_INSECURE_ADMIN_PUSH` (CP side) | The CP-side half of the RLS admin channel (pair with ¹⁹ on the RLS). **Fail-closed**: a configured push token + a non-`https://` `FLOWPLANE_RLS_ADMIN_URL` refuses startup unless the URL host is a loopback **literal** (`localhost`, `127.0.0.0/8`, `::1`, `::ffff:127.0.0.0/8` — never DNS-resolved) *and* the push escape hatch is set; an unparseable URL with a token always refuses. Set the same token value on both sides. With no token the push is unauthenticated (loopback dev pairing with the RLS admin hatch). |
| ²¹ | `FLOWPLANE_CERT_ISSUER_CA_CERT_PATH`, `FLOWPLANE_CERT_ISSUER_CA_KEY_PATH` | Validated when certificate issuance is invoked, before registry/audit/outbox mutation. Certificate and private key must parse and match; the certificate must be currently valid and contain `CA:TRUE`, `keyCertSign`, and a Subject Key Identifier, with no EKU restriction excluding `clientAuth`. Root and explicit intermediate trust anchors are supported. Failure is `invalid_config`; API details remain redacted while operator logs retain the reason. See [issuer CA compatibility and upgrade](../how-to/production-readiness.md#issuer-ca-compatibility-and-upgrade). |

## Agent diagnostics recovery

The daemon treats `/healthz` as a readiness signal, not a process-liveness signal. It returns `503` when Envoy polling or accepted diagnostics acknowledgments are stale, while the process remains alive and retries temporary connection, stream, send, receive, EOF, and attempt-timeout failures. The source queue remains bounded by `FLOWPLANE_AGENT_QUEUE_CAP`; one additional report may be retained in flight for exact-ID replay. `--once`, invalid startup configuration, explicit invalid or unauthorized acknowledgments, and mismatched acknowledgment IDs remain fatal.

Every reconnect rebuilds the diagnostics channel and rereads the CA, client certificate, and private key files from the configured paths. Replace all related files atomically before the next attempt. A valid replacement is used without restarting the agent; an invalid or untrusted replacement fails closed and leaves readiness degraded.

After the control plane is functional and accepts and commits reports, use this conservative readiness-recovery bound: the remaining current report-attempt deadline, plus the maximum jittered backoff of 6 seconds, plus one complete successful report-attempt deadline, plus at most one poll interval for a newly committed heartbeat after an idempotent replay. This is a conservative operational bound, not a guarantee while the CP listener cannot commit to its database. The deadline and retry schedule are internal safety policy rather than configuration variables.
