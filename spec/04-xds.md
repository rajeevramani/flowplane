...
Behavior:
- **Poll loop**: every `poll_interval` (default 10 s, min 1 s), GET Envoy admin `/stats?format=json`. The `stats` array is parsed tolerantly — only `{name, numeric value}` entries are kept; the non-`{name,value}` histograms element (`{"histograms": …}`) and any string/null/float values are skipped, so one non-scalar entry cannot fail deserialization of the whole payload (shape-based and version-agnostic across Envoy releases — see #170). Entries whose name ends with `.downstream_rq_total` are summed into requests and `.downstream_rq_5xx` into errors; the per-cycle delta is the non-negative difference from the previous snapshot. A scrape or parse failure (incl. a malformed body — invalid JSON or `stats` missing / not an array) logs a warning and retries next interval without emitting a report.
- **Heartbeat**: every successful cycle emits a `HeartbeatReport` (`DiagnosticsReport` schema 1, UUIDv7 `report_id`, `observed_at` unset) carrying `requests_delta` / `errors_delta`; `warming_failures_delta` is currently always 0 and `config_verified` always true. Reports enter a bounded FIFO queue (`FLOWPLANE_AGENT_QUEUE_CAP`, default 256). The current agent does **not** scrape `/config_dump` or emit `ListenerStateReport` — the `ListenerState` payload variant exists in the diagnostics contract and is accepted by the CP, but is not produced by the agent today.
- **Stream loop**: connect to `FLOWPLANE_AGENT_CP_ENDPOINT` (5 s connect timeout; mandatory mTLS when `FLOWPLANE_AGENT_TLS_{CERT,KEY,CA}_PATH` are set, otherwise plaintext — permitted only for a loopback CP endpoint), open the `report_diagnostics` bidi stream, and drain the queue into it. An inbound `Ack` with status ≠ `Ok` aborts the stream (error → supervisor restart); a successful Ack records `last_ack`.
- **Health**: `GET /healthz` on `FLOWPLANE_AGENT_HEALTH_BIND_ADDR` (default `127.0.0.1:19902`) → 200 only when **both** the last successful admin poll and the last successful Ack are within `max(2 × poll_interval, 2 s)`; otherwise 503 with a reason (`never polled`, `admin poll stale {n}s`, `never acked`, or `diagnostics ack stale {n}s`).
- **Supervisor**: the poll loop, stream loop, and health server run concurrently; if any one returns, the process exits (non-zero on error) for an external supervisor to restart. `--once` performs a single scrape → heartbeat → Ack and exits 0.
