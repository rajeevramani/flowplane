...
xDS port 18000 is not a Fly public service. Tailscale userspace networking exposes the stable tailnet-only Service `svc:fpq-flowplane-cp-g2` as a raw TCP forwarder to Flowplane's loopback port 18000. Tailscale does not terminate TLS: Flowplane terminates xDS mTLS and authorizes the dataplane client certificate. Dataplanes pin the stable Service FQDN `fpq-flowplane-cp-g2.tail67f704.ts.net`, which must also appear in the xDS certificate SAN; individual Fly node DNS names may gain numeric suffixes after replacement and must never be pinned.
...
The tailnet administrator must define both qualification Services, permit dataplanes to reach CP `tcp:18000` and RLS `tcp:50051`, and auto-approve advertisements from `tag:fpq-cp-service` and `tag:fpq-rls-service`. Each Tailscale auth key must be short-lived, reusable, ephemeral, pre-authorized, and carry only its role tag. Each boot re-advertises its Service because Fly's root filesystem and `tailscaled.state` are disposable. Clients pin the stable Service FQDNs, never individual node names or TailVIPs.
