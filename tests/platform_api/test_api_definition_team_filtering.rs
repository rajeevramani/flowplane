...
/// Test that team-scoped users cannot access bootstrap for other teams' API definitions
#[tokio::test]
async fn team_scoped_user_cannot_get_bootstrap_for_other_team() {
    let app = setup_platform_api_app().await;
    let admin_token = app.issue_token("platform-admin", &["admin:all"]).await;
    ...
    // Try to get bootstrap as payments user
    let payments_token =
        app.issue_token("payments-user", &["team:payments:api-definitions:read"]).await;
    let response = send_request(
        &app,
