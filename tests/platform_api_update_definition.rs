...
async fn platform_api_patch_updates_propagate_to_envoy() {
    ...
    let patch_uri: hyper::http::Uri =
        format!("http://{}/api/v1/api-definitions/{}", api_addr, definition_id)
            .parse()
            .unwrap();
    ...
    let stored_domain: String = sqlx::query_scalar("SELECT domain FROM api_definitions WHERE id = ?")
        .bind(definition_id)
        .fetch_one(&pool)
        .await
        .expect("fetch domain");
...
async fn platform_api_patch_target_listeners_propagates_to_envoy() {
    ...
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    let body = envoy.wait_for_route(&domain, "/api/test", 200).await.expect("initial routing failed");
    ...
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    let body = envoy
        .wait_for_route(&domain, "/api/test", 200)
        .await
        .expect("routing failed after PATCH");
