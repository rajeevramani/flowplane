...
- [ ] S12 Hardening, production readiness, v1.0.0 tag
  ...
  - [x] S12e packaging (greenfield release infrastructure): #93 adds `scripts/package-release.sh`,
        ...
        The v1.0 artifact decision is glibc/container packaging, not musl-static, because first-party
        crates directly depend on OpenSSL. The default package command produces host binaries,
        cargo-metadata SBOM source, tarball, manifest, and checksums; `FLOWPLANE_PACKAGE_IMAGE=1`
        builds/saves the OCI image when Docker/Podman has free builder storage. Dataplane bootstrap
        remains generated by the shipped `flowplane dataplane bootstrap` command against a live CP.
